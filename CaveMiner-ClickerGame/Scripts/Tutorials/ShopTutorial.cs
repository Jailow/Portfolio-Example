...
namespace CaveMiner
{
    public class ShopTutorial : TutorialBase
    {
        ...
        [SerializeField] private Image _shopButtonDisabledTexture;
        ...
        protected override IEnumerator Tutorial()
        {
            ...
            var movePos = _shopButtonDisabledTexture.transform.position;
            movePos.z = 0f;

            var textTabPos = _shopButtonDisabledTexture.transform.position;
            textTabPos.z = 0f;

            _firstTextTabTr.position = textTabPos;

            _highlight.DOMove(movePos, 1f).SetEase(Ease.InOutSine);
            _highlight.DOSizeDelta(_shopButton.image.rectTransform.sizeDelta, 1f).SetEase(Ease.InOutSine);

            yield return new WaitForSeconds(1f);

            _shopButtonDisabledTexture.DOFade(0f, 0.5f);

            yield return new WaitForSeconds(0.5f);

            _shopButtonDisabledTexture.gameObject.SetActive(false);
            ...
            _shopButton.onClick.AddListener(OnButtonClicked);

            while (!_buttonClicked)
                yield return null;

            _shopButton.onClick.RemoveListener(OnButtonClicked);
