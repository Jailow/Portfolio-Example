...
namespace CaveMiner
{
    public class TasksTutorial : TutorialBase
    {
        [SerializeField] private GameObject _lockControl;
        [SerializeField] private RectTransform _taskButtonHighlightPos;
        [SerializeField] private RectTransform _highlight;
        [SerializeField] private UITutorialTab _tasksButton;
        ...
        protected override IEnumerator Tutorial()
        {
            ...
            var movePos = _taskButtonHighlightPos.position;
            movePos.z = 0f;

            _highlight.DOMove(movePos, 1.5f).SetEase(Ease.InOutSine);
            _highlight.DOSizeDelta(_taskButtonHighlightPos.sizeDelta, 1.5f).SetEase(Ease.InOutSine);
            _tasksButton.ShowAnimation();
            ...
            if (gameManager.GameState.PlayerState.Tasks.Count <= 0)
            {
                gameManager.GenerateNewCaveTasks("C001", 3);
            }

            _lockControl.SetActive(false);
            var btn = _tasksButton.GetComponent<Button>();
