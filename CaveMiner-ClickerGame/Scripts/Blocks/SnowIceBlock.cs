...
namespace CaveMiner
{
    public class SnowIceBlock : BlockBase
    {
            ...
            for (int i = 0; i < destroyCount; i++)
            {
                ...
                if (_currentIndex < _newPartsFromIndex) // Первый тип частей
                {
                    if (explosion)
                    {
                        ...
                        _parts[_currentIndex].transform.DOScale(0f, 0.25f);
                    }
                    else
                    {
                        _parts[_currentIndex].DOFade(0f, 0.5f);
                        _parts[_currentIndex].transform.DOScale(0f, 0.5f);
                    }
                }
                else // Второй тип частей
                {
                    if (explosion)
                    {
                        _parts[_currentIndex].DOFade(0f, 0.3f);
                    }
                    else
                    {
                        _parts[_currentIndex].DOFade(0f, 0.5f);
